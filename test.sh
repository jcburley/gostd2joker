...
./gostd2joker --notime -v --go tests/small 2>&1 | grep -v '^Default context:' > $GOENV/small.gold
git diff --quiet -u $GOENV/small.gold || { echo >&2 "FAILED: small test"; RC=1; $EXIT; }

rm -fr tests/joker
cp -pr tests/joker.orig tests/joker
./gostd2joker --notime -v --go tests/big --replace --joker tests/joker 2>&1 | grep -v '^Default context:' > $GOENV/big.gold
...
if [ -n "$GOSRC" -a -d "$GOSRC" ]; then
    ./gostd2joker --notime -v --go "$GOSRC" 2>&1 | grep -v '^Default context:' > $GOENV/gosrc.gold
